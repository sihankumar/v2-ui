...
update() {
    ...
    bash <(curl -Ls https://github.com/sihankumar/v2-ui.sh)
    if [[ $? == 0 ]]; then
...
uninstall() {
    ...
    echo ""
    echo -e "Telegram 群组: ${green}https://t.me/sihananushkavpn${plain}"
    echo -e "Github issues: ${green}https://github.com/sihankumar/issues${plain}"
    echo -e "博客: ${green}https://github.com/sihankumar${plain}"
...
install_bbr() {
    bash <(curl -L -s https://github.com/sihankumar/raw/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://github.com/sihankumar/v2-ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
